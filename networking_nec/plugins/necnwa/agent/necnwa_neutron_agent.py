...
from networking_nec.plugins.necnwa.agent.necnwa_fwaas_callback \
    import NECNWAFirewallCallback
from networking_nec.plugins.necnwa.agent.necnwa_lbaas_callback \
    import NECNWALBaaSCallback
...
def check_segment_gd(network_id, res_name, nwa_data):
    return check_segment(network_id, res_name, nwa_data,
                         necnwa_utils.NWA_DEVICE_GDV)


def check_segment_tfw(network_id, res_name, nwa_data):
    return check_segment(network_id, res_name, nwa_data,
                         necnwa_utils.NWA_DEVICE_TFW)
...
class NECNWANeutronAgent(object):
    ...
    def setup_rpc(self):
        ...
        if self.conf.NWA.lbaas_driver:
            ...
            self.callback_lbaas = NECNWALBaaSCallback(self.context,
                                                      self.lbaas_driver)
        else:
        ...
        if self.conf.NWA.fwaas_driver:
            ...
            self.callback_fwaas = NECNWAFirewallCallback(self.context,
                                                         self.fwaas_driver)
        else:
    ...
    def delete_tenant_rpc_server(self, tid):
        if tid not in self.rpc_servers.keys():
            LOG.warning(
                _LW("rpc server not found. tid=%s") % tid
            )
            return {'result': 'FAILED'}
    ...
    def _report_state(self):
        try:
            ...
            LOG.error(_LE("Failed reporting state!"))
    ...
    def create_tenant_fw(self, context, **kwargs):
        ...
        if dev_key not in nwa_data.keys():
            ...
            if (
                ...
                nwa_data[nid + '_' +
                         resource_group_name_nw] = necnwa_utils.NWA_DEVICE_TFW
                # for ext net
                ...
                if self.fwaas_driver:
                    ...
                    # set firewall(draft)
                    self._setting_fw_policy_all_permit(context,
                                                       tfw_name, **kwargs)
            ...
            LOG.info(_LI("_update_tenant_fw.ret_val=%s") % jsonutils.dumps(
                body,
                indent=4,
                sort_keys=True
            ))
            if rcode is False:
                LOG.error(_LE("UpdateTenantFW FAILED."))
                return self._update_tenant_binding(
                    context, tenant_id, nwa_tenant_id, nwa_data,
                    nwa_created=nwa_created
                )
        ...
        vlan_id = int(nwa_data['VLAN_' + network_id +
                               '_' + resource_group_name_nw +
                               VLAN_OWN_TFW + '_VlanID'], 10)
    ...
    def _delete_tenant_fw(self, context, **kwargs):
        ...
        if (
            ...
            if not check_segment_tfw(network_id, resource_group_name_nw,
                                     nwa_data):
                nwa_data.pop(vp_net)
                nwa_data.pop(vp_net + '_VlanID')
    ...
    def delete_tenant_fw(self, context, **kwargs):
        ...
        # delete vlan
        result, ret_val = self._delete_vlan(context,
                                            nwa_data=nwa_data, **kwargs)
        LOG.info(_LI("_delete_vlan_fw.ret_val=%s") % jsonutils.dumps(
            ret_val,
            indent=4,
            sort_keys=True
        ))
        ...
        result, ret_val = self._delete_tenant_nw(context,
                                                 nwa_data=nwa_data, **kwargs)
        ...
        # delete tenant
        result, ret_val = self._delete_tenant(context,
                                              nwa_data=nwa_data, **kwargs)
        LOG.info(_LI("_delete_tenant.ret_val=%s") % jsonutils.dumps(
            ret_val,
            indent=4,
            sort_keys=True
        ))
    ...
    def _append_device_for_gdv(self, nwa_info, nwa_data):
        ...
        nwa_data[net_key + '_' +
                 resource_group_name] = necnwa_utils.NWA_DEVICE_GDV
    ...
    def setting_nat(self, context, **kwargs):
        ...
        rcode, ret_val = self._setting_nat(context,
                                           nwa_data=nwa_data, **kwargs)

...
from networking_nec._i18n import _LE, _LW  # noqa
...
class NECNWAAgentBaseDriver(object):
    ...
    def nwa_create_tenant_lb(self, context, tid, nwa_tid, net_id, pool_id,
                             vip_id, address, mac, vlan_type):
        nwa_data = self.get_nwa_tenant_binding(context, tid, nwa_tid)
    ...
    def nwa_update_tenant_lb_add(self, context, tid, nwa_tid, net_id, pool_id,
                                 vip_id, address, mac, vlan_type):
        nwa_data = self.get_nwa_tenant_binding(context, tid, nwa_tid)
    ...
    def nwa_update_tenant_lbn(
        ...
        if vip_key not in nwa_data.keys():
            msg = "vip key not found. key=%s" % vip_key
            raise NWADriverError(api="UpdateTenantLB(disconnect,connect)",
                                 msg=msg)

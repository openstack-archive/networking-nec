...
class NECNWAServicePlugin(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def _delete_nat(self, context, fip):
        ...
        LOG.info('delete_nat fl_data={}'.format(fl_data))
    ...
    def disassociate_floatingips(self, context, port_id, do_notify=True):
        LOG.debug('context.session={} port_id={}'.format(context.session, port_id))
        try:
            ...
            LOG.warning('floatingip not found {}'.format(port_id))
    ...
    def update_floatingip(self, context, id, floatingip):
        ...
        LOG.debug('context={} id={} floatingip={}'.format(context, id, floatingip))
        ...
        try:
            if update_flag is False:
                floating = context.session.query(l3_db.FloatingIP).filter_by(id=id).one()
        ...
        try:
            if update_flag is True:
                floating = context.session.query(l3_db.FloatingIP).filter_by(id=id).one()
                tenant_id = nwa_utils.get_tenant_id_by_router(context.session, floating['router_id'])
                nwa_tenant_id = nwa_utils.get_nwa_tenant_id(tenant_id)

                fl_data = {'floating_ip_address': floating['floating_ip_address'],
                ...
                LOG.info('setting_nat fl_data={}'.format(fl_data))
                proxy = self._core_plugin.get_nwa_proxy(tenant_id)
                proxy.setting_nat(
                    context, tenant_id=tenant_id,
                    nwa_tenant_id=nwa_tenant_id,
    ...
    def add_router_interface(self, context, router_id, interface_info):
        ...
        ret = super(NECNWAServicePlugin, self).add_router_interface(context, router_id, interface_info)

        if nwa_utils.add_router_interface_by_port(self, context, router_id, interface_info) is False:

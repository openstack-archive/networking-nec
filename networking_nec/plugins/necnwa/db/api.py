    ...
    try:
        ...
        with session.begin(subtransactions=True):
            for json_key, json_value in json_value.items():
                item = nmodels.NWATenantBindingN(tenant_id, nwa_tenant_id, json_key, json_value)
                session.add(item)
            session.flush()
        return True
    ...
    try:
        ...
        for nwa in session.query(nmodels.NWATenantBindingN).filter(
            nmodels.NWATenantBindingN.tenant_id == tenant_id).filter(
                nmodels.NWATenantBindingN.nwa_tenant_id == nwa_tenant_id).all():
            value_json[nwa.json_key] = chg_value(nwa.json_key, nwa.json_value)
        if len(value_json):
            return nmodels.NWATenantBinding(tenant_id, nwa_tenant_id, value_json)
        else:
            return None
    except sa.orm.exc.NoResultFound:
        return None
    ...
    with session.begin(subtransactions=True):
        for key, value in value_json.items():
            if key in _json:
                ...
                # insert
                # item = nmodels.NWATenantBindingN(tenant_id, nwa_tenant_id, key, value)
                # session.add(item)
                insert = ("INSERT INTO nwa_tenant_binding (tenant_id,nwa_tenant_id,json_key,json_value) "
                          " VALUES (\'%s\',\'%s\',\'%s\',\'%s\') ON DUPLICATE KEY UPDATE "
                          " json_value=\'%s\'" % (tenant_id, nwa_tenant_id, key, value, value))
        ...
        with session.begin(subtransactions=True):
            ...
            with session.begin(subtransactions=True):
                session.query(nmodels.NWATenantBindingN).filter(
                    and_(nmodels.NWATenantBindingN.tenant_id == tenant_id,
                         nmodels.NWATenantBindingN.nwa_tenant_id == nwa_tenant_id)).delete()
            session.flush()
            return True
    except sa.orm.exc.NoResultFound:
        return False
...
# address  ... ip address
def add_nwa_address_member(session, type, policy_id, rule_id, subnet, address, id=0):
    try:

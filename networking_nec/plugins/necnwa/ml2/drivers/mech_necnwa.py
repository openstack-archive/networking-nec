...
from neutron.plugins.ml2.drivers.openvswitch.mech_driver import mech_openvswitch as ovs
...
class NECNWAMechanismDriver(ovs.OpenvswitchMechanismDriver):
    ...
    def create_port_precommit(self, context):
        try:
            ...
            if ((device_owner == constants.DEVICE_OWNER_ROUTER_INTF or
                ...
                if not device_owner in [res_grp['device_owner'] for res_grp in grp]:
                    LOG.warning("resource group miss match. device_owner=%s" % (device_owner))
                    return
                ...
                LOG.warning("device owner missmatch device_owner=%s" % device_owner)
                return
    ...
    def delete_port_precommit(self, context):
        ...
        try:
            ...
            LOG.debug(_("tenant_id=%(tid)s, nwa_tenant_id=%(nid)s, device_owner=%(dev)s"),
    ...
    def try_to_bind_segment_for_agent(self, context, segment, agent):
        ...
        for res in grp:
            ...
            if res['device_owner'] == context._port['device_owner']:
                ...
                if dummy_segment is None:
                    dummy_segment = {api.PHYSICAL_NETWORK: res['ResourceGroupName'],
                                     api.NETWORK_TYPE: n_constants.TYPE_VLAN,
                                     api.SEGMENTATION_ID: 0}
                    db.add_network_segment(
                        context.network._plugin_context.session,
        ...
        LOG.warning(_("binding segment not found for agent=%s"),
                    agent)
        ...
        try:
            ...
            if prov_net.PHYSICAL_NETWORK in context.network.current:
                if context.network.current[prov_net.SEGMENTATION_ID] is None:
                    ...
                    LOG.debug("current segmentation_id = %d" % context.network.current[prov_net.SEGMENTATION_ID])
            else:
                for provider in context.network.current['segments']:
                    physical_network = nwa_utils.get_physical_network(device_owner)
                    if not provider[prov_net.PHYSICAL_NETWORK] == physical_network:
                        continue
                    segmentation_id = provider[prov_net.SEGMENTATION_ID]
                    LOG.debug("provider segmentation_id = %d" % segmentation_id)
                    break

            LOG.debug(_("_bind_port_nwa %(tenant_id)s %(network_name)s "
            ...
            LOG.debug(_('bind_port recode=%(recode)s'), {'recode': recode})

            nwa_info = nwa_utils.portcontext_to_nwa_info(context)
            proxy = context._plugin.get_nwa_proxy(tenant_id)
            ret = proxy.create_general_dev(
                context.network._plugin_context,
                tenant_id,
                nwa_tenant_id,
                nwa_info

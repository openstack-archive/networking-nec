...
from neutron_fwaas.services.firewall.fwaas_plugin import (
    FirewallPlugin,
    FirewallCallbacks
)
...
class NECNWAFirewallPlugin(
    ...
    def update_firewall(self, context, id, firewall):
        ...
        return super(NECNWAFirewallPlugin, self).update_firewall(context, id, firewall)
    ...
    def update_firewall_policy(self, context, id, firewall_policy):
        LOG.debug("[NWA] update_firewall_policy id=%s, firewall_policy=%s" % (id, firewall_policy))
        return super(NECNWAFirewallPlugin, self).update_firewall_policy(context, id, firewall_policy)

    def update_firewall_rule(self, context, id, firewall_rule):
        ret = super(NECNWAFirewallPlugin, self).update_firewall_rule(context, id, firewall_rule)
        LOG.debug("id=%s" % id)
        LOG.debug("firewall_rule=%s" % json.dumps(
            firewall_rule,
            indent=4,
            sort_keys=True
        ))
    ...
    def insert_rule(self, context, id, rule_info):
        ret = super(NECNWAFirewallPlugin, self).insert_rule(context, id, rule_info)
        LOG.debug("id=%s" % id)
        LOG.debug("rule_info=%s" % json.dumps(
            rule_info,
            indent=4,
            sort_keys=True
        ))
    ...
    def remove_rule(self, context, id, rule_info):
        ret = super(NECNWAFirewallPlugin, self).remove_rule(context, id, rule_info)
        LOG.debug("id=%s" % id)
        LOG.debug("rule_info=%s" % json.dumps(
            rule_info,
            indent=4,
    ...
    def _get_nwa_fwaas_proxy(self, tid):
        if not tid in self.nwa_fwaas_proxies.keys():

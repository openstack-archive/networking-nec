...
def baremetal_resource_group_name(mac_address):
    try:
        for pmap in jsonutils.loads(config.CONF.NWA.PortMap):
...
def _set_segment_to_tenant_binding(context, jbody):
    try:
        ...
        if seg_key not in nwa_data.keys():
            nwa_data['VLAN_' + network_id + '_' +
                     physical_network] = 'physical_network'
            nwa_data['VLAN_' + network_id + '_' + physical_network +
                     '_segmentation_id'] = jbody['resultdata']['VlanID']
            nwa_data['VLAN_' + network_id + '_' + physical_network +
                     '_VlanID'] = jbody['resultdata']['VlanID']
...
def _set_general_dev_to_tenant_binding(context):
    try:
        ...
        nwa_data['DEV_' + device_id +
                 '_device_owner'] = context._port['device_owner']
        nwa_data['DEV_' + device_id + '_' + network_id] = network_name
        nwa_data['DEV_' + device_id + '_' + network_id +
                 '_TYPE'] = NWA_DEVICE_GDV
        nwa_data['DEV_' + device_id + '_' + network_id +

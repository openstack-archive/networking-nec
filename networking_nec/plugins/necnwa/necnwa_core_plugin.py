...
class NECNWAPluginTenantBinding(object):
    ...
    def update_port_state_with_notifire(self, context, device, agent_id,
    ...
    def get_nwa_proxy(self, tid, context=None):
        if tid not in self.nwa_proxies.keys():
            ...
            if context:
                ...
                if len(topics) == 1:
                    LOG.info(_LI('NWA tenant queue: new topic is {}')
                             .format(str(topics[0])))
                else:
                    LOG.warning(_LW('NWA tenant queue is not created. tid={}')
                                .format(tid))

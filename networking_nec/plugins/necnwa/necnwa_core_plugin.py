...
class NECNWAPluginTenantBinding(object):
    ...
    def update_port_state_with_notifire(self, context, device, agent_id, port_id, segment, network_id):
        ...
        return cctxt.call(
            context,
            'update_port_state_with_notifire',
            device=device,
            agent_id=agent_id,
            port_id=port_id,
            network_id=network_id,
    ...
    def release_dynamic_segment_from_agent(self, context, physical_network, network_id):
        cctxt = self.client.prepare()
        return cctxt.call(
            context,
...
class NECNWARpcCallbacks(rpc.RpcCallbacks):
    ...
    def get_device_details(self, rpc_context, **kwargs):
        ...
        if not segment:
            ...
            LOG.warning(_("Device %(device)s requested by agent "
                          "%(agent_id)s, segment %(segment_id)s has network %(network_id)s not "
                          "no segment from NWA"),
                        {'device': device, 'agent_id': agent_id,
    ...
    def get_nwa_tenant_binding(self, rpc_context, **kwargs):
        ...
        LOG.debug("tenant_id=%s, nwa_tenant_id=%s" % (tenant_id, nwa_tenant_id))
        session = db_api.get_session()
        with session.begin(subtransactions=True):
            recode = necnwa_api.get_nwa_tenant_binding(
                session, tenant_id, nwa_tenant_id
            )
            if not recode is None:
                LOG.debug(
                    "nwa_data=%s" % json.dumps(recode.value_json, indent=4, sort_keys=True)
                )
    ...
    def set_nwa_tenant_binding(self, rpc_context, **kwargs):
        ...
        LOG.debug("tenant_id=%s, nwa_tenant_id=%s" % (tenant_id, nwa_tenant_id))
        LOG.debug(
            "nwa_data=%s" % json.dumps(nwa_data, indent=4, sort_keys=True)
        )
        ...
        with session.begin(subtransactions=True):
            ...
            for queue in queues:
                ...
                if queue.tenant_id in tenant_ids:
                    LOG.info("RPC Server active(tid=%s)" % queue.tenant_id)
                    continue
                else:
    ...
    def update_port_state_with_notifire(self, rpc_context, **kwargs):
        device = kwargs.get('device')
        agent_id = kwargs.get('agent_id')
        port_id = kwargs.get('port_id')
        network_id = kwargs.get('network_id')
        ...
        with session.begin(subtransactions=True):
            try:
                ...
                LOG.error(_("Can't find port with port_id %s"),
                          port_id)
        ...
        # 4 send notifier
        if not port is None:
            LOG.debug("notifier port_update %s, %s, %s" % (network_type, segmentation_id, physical_network))
...
class NECNWACorePlugin(ml2_plugin.Ml2Plugin):
    ...
    def start_rpc_listeners(self):
        self.endpoints = [NECNWARpcCallbacks(self.notifier, self.type_manager),
    ...
    def _extend_network_dict_provider(self, context, network):
        ...
        if not segments:
            LOG.debug(_("Network %s has no segments"), id)
            network[provider.NETWORK_TYPE] = None
            network[provider.PHYSICAL_NETWORK] = None
            network[provider.SEGMENTATION_ID] = None
        elif len(segments) > 1:
    ...
    def _create_nwa_agent_tenant_queue(self, context, tenant_id):
        if (
            ...
            LOG.warning('%s is not alive.' % necnwa_agent_rpc.NWA_AGENT_TYPE)
    ...
    def delete_network(self, context, id):
        ...
        if (
                self._is_alive_nwa_agent(context) and
                necnwa_api.get_nwa_tenant_queue(
                    context.session, context.tenant_id
                )
        ):
            self.nwa_rpc.delete_server(context, context.tenant_id)
            necnwa_api.del_nwa_tenant_queue(context.session, context.tenant_id)
        else:
            LOG.warning('%s is not alive.' % necnwa_agent_rpc.NWA_AGENT_TYPE)
        """
    ...
    def get_nwa_proxy(self, tid, context=None):
        if tid not in self.nwa_proxies.keys():
            ...
            if context:
                ...
                if len(topics) == 1:
                    LOG.info('NWA tenant queue: new topic is {}'
                             .format(str(topics[0])))
                else:
                    LOG.warning('NWA tenant queue is not created. tid={}'
                                .format(tid))
        LOG.debug('proxy tid={}'.format(tid))
    ...
    def _is_alive_nwa_agent(self, context):
        ...
        if(
                0 < len(agents) and
                True in [v['alive'] for v in agents]
        ):
            return True

...
from neutron_fwaas.services.firewall.agents.firewall_agent_api \
    import FWaaSPluginApiMixin
from oslo_log import log as logging
from oslo_serialization import jsonutils

from networking_nec.plugins.necnwa.agent.base_driver import \
    NECNWAAgentBaseDriver
from networking_nec.plugins.necnwa.agent.base_driver import POLICY_CREATE
from networking_nec.plugins.necnwa.agent.base_driver import POLICY_DELETE
from networking_nec.plugins.necnwa.agent.base_driver import POLICY_PERMIT
from networking_nec.plugins.necnwa.agent.base_driver import POLICY_RULE
from networking_nec.plugins.necnwa.agent.base_driver import POLICY_UPDATE
...
class NECNWAFirewallAgentDriver(NECNWAAgentBaseDriver):
    ...
    def create_firewall(self, context, **kwargs):
        ...
        for router_id in router_ids:
            if 'DEV_' + router_id + '_TenantFWName' in nwa_data.keys():
                ...
                self.nwa_setting_fw_policy(context, tid, nwa_tid, tfw,
    ...
    def update_firewall(self, context, **kwargs):
        ...
        for router_id in firewall['router_ids']:
            rt_key = 'DEV_' + router_id + '_TenantFWName'
            if rt_key in nwa_data:
                # set rules
                LOG.debug("set policy in router.")
                self.nwa_setting_fw_policy(context, tid, nwa_tid,
                                           nwa_data[rt_key],
                                           firewall, POLICY_RULE,
                                           POLICY_UPDATE)

        for router_id in firewall['add-router-ids']:
            if router_id in router_ids:
                continue

            router_key = 'DEV_' + router_id + '_TenantFWName'
            if router_key in nwa_data:
                # set all deny
                LOG.debug("set policy all deny in router.")
                self.nwa_setting_fw_policy(context, tid, nwa_tid,
                                           nwa_data[router_key],
                                           firewall, POLICY_RULE,
                                           POLICY_UPDATE)

        for router_id in firewall['del-router-ids']:
            rt_key = 'DEV_' + router_id + '_TenantFWName'
            if rt_key in nwa_data:
                # set all permit

...
class NECNWAFirewallPlugin(FirewallPlugin):

    supported_extension_aliases = ["fwaas", "fwaasrouterinsertion"]

    def __init__(self):
        super(NECNWAFirewallPlugin, self).__init__()
    ...
    def update_firewall(self, context, id_, firewall):
        LOG.debug("[NWA] update_firewall %s %s" % (id_, firewall))
        return super(NECNWAFirewallPlugin, self).update_firewall(context,
                                                                 id_, firewall)

    def delete_firewall(self, context, id_):
        LOG.debug("[NWA] delete_firewall %s" % id_)
        return super(NECNWAFirewallPlugin, self).delete_firewall(context, id_)

    def update_firewall_policy(self, context, id_, firewall_policy):
        LOG.debug("[NWA] update_firewall_policy id=%s, firewall_policy=%s" %
                  (id_, firewall_policy))
        return super(NECNWAFirewallPlugin, self).update_firewall_policy(
            context, id_, firewall_policy)

    def update_firewall_rule(self, context, id_, firewall_rule):
        ret = super(NECNWAFirewallPlugin, self).update_firewall_rule(
            context, id_, firewall_rule)
        LOG.debug("id=%s" % id_)
    ...
    def insert_rule(self, context, id_, rule_info):
        ret = super(NECNWAFirewallPlugin, self).insert_rule(context, id_,
                                                            rule_info)
        LOG.debug("id=%s" % id_)
    ...
    def remove_rule(self, context, id_, rule_info):
        ret = super(NECNWAFirewallPlugin, self).remove_rule(context, id_,
                                                            rule_info)
        LOG.debug("id=%s" % id_)
        LOG.debug("rule_info=%s" % jsonutils.dumps(
            rule_info,
    ...
    def _get_nwa_fwaas_proxy(self, tid):
        if tid not in self.nwa_fwaas_proxies.keys():
            self.nwa_fwaas_proxies[tid] = NECNWAFirewallAgentApi(
                nwa_const.NWA_AGENT_TOPIC, cfg.CONF.host, tid
            )

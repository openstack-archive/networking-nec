...
from networking_nec._i18n import _LI, _LW  # noqa
    ...
    def rest_api(self, method, url, body=None):
        ...
        try:
            ...
            if 200 <= status_code and status_code <= 209:
                ...
                msg = _LW("NWA failed: {0} {1}:{2}{3} "
                          "(HTTP/1.1 {4} {5}) body={6}").format(
    ...
    def _report_workflow_error(self, data, errno):
        ...
        msg = ("NWA workflow: {0} reason({1})={2} request={3}, response={4}"
               .format(name, errno, reason, post_body,
                       jsonutils.dumps(data, indent=4, sort_keys=True)))
        return msg
...
class NwaRestClient(RestClient):
    ...
    def __init__(self, *args, **kwargs):
        ...
        LOG.info(
            _LI('NWA init: host={0} port={1} use_ssl={2} auth={3}')
            .format(self.host, self.port, self.use_ssl, self.auth)
        )
        LOG.info(_LI('NWA init: workflow wait: {0}s + {1}s x {2} times.')
                 .format(self.workflow_first_wait,
                         self.workflow_wait_sleep,
                         self.workflow_retry_count))
        LOG.info(_LI('NWA init: umf api version: {0}').format(
...
class NwaClient(NwaRestClient):
    ...
    def workflow_kick_and_wait(self, call, url, body):
        ...
        try:
            ...
            LOG.warning(_LW('NWA workflow: retry over. retry count is {0}.')
...
def test_ng(ctx, http_status, rj, *args, **kwargs):   # pragma: no cover
    ...
    # print(jsonutils.dumps(rj, indent=4, sort_keys=True))
    print('ng'),
    print(jsonutils.dumps({
        'context': ctx,
        'http_status': http_status,
        ...
        if nargs < 3:
            ...
            try:
                with open(pname) as f:
                    props = jsonutils.load(f)
            except IOError:
                pass
            if props is None:
                try:
                    props = jsonutils.loads(pname)
                except Exception as e:
                    print('arg2: JSON Error: {}'.format(str(e)))
                    sys.exit(1)

            props = overwrite_with_args(props, args)
            print(jsonutils.dumps(props, indent=4, sort_keys=True))
            rd = nwa.setting_fw_policy(tenant_id, fw_name, props)
            print(jsonutils.dumps(rd, indent=4, sort_keys=True))

...
FLOATINGIP_FIELD_MAP = {
    'NAT_%(fip_id)s': 'device_id',
    'NAT_%(fip_id)s_network_id': 'floating_network_id',
    'NAT_%(fip_id)s_floating_ip_address': 'floating_ip_address',
    'NAT_%(fip_id)s_fixed_ip_address': 'fixed_ip_address'
}


def set_floatingip_data(nwa_data, floating):
    fip_id = floating['id']
    for key_fmt, from_key in FLOATINGIP_FIELD_MAP.items():
        key = key_fmt % {'fip_id': fip_id}
        nwa_data[key] = floating[from_key]


def strip_floatingip_data(nwa_data, floating):
    fip_id = floating['id']
    for key_fmt in FLOATINGIP_FIELD_MAP:
        key = key_fmt % {'fip_id': fip_id}
        nwa_data.pop(key)

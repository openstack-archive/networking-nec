...
from neutron.plugins.nec.db import router as rt_model
...
def _get_router_providers_query(query, provider=None, router_ids=None):
    ...
    if router_ids:
        column = rt_model.RouterProvider.router_id
...
def get_router_providers(session, provider=None, router_ids=None):
    ...
    query = session.query(rt_model.RouterProvider)
...
def get_routers_by_provider(session, provider, router_ids=None):
    ...
    query = session.query(rt_model.RouterProvider.router_id)
    query = _get_router_providers_query(query, provider, router_ids)
...
def get_router_count_by_provider(session, provider, tenant_id=None):
    ...
    query = session.query(rt_model.RouterProvider).filter_by(provider=provider)
    if tenant_id:
...
def get_provider_by_router(session, router_id):
    ...
    try:
        binding = (session.query(rt_model.RouterProvider).
...
def add_router_provider_binding(session, provider, router_id):
    ...
    binding = rt_model.RouterProvider(provider=provider, router_id=router_id)
    session.add(binding)
    return binding

...
def check_vlan(network_id, nwa_data):
    ...
    vlan_pattern = re.compile(r'VLAN_.*' + network_id + '_.*_VlanID$')
    # vlan_pattern = re.compile(r'VLAN_.*_' + network_id + '_.*_VlanID$')
    matched = [k for k in nwa_data if vlan_pattern.match(k)]
    if matched:

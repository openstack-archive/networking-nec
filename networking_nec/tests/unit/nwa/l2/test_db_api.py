...
class TestAddNwaTenantBinding(testlib_api.SqlTestCaseLight):
    ...
    def test_add_nwa_tenant_binding_no_result_found(self):
        self.ssn.query().filter().all.side_effect = NoResultFound
        self.assertFalse(
            db_api.add_nwa_tenant_binding(
                self.ssn, self.tenant1, self.nwa_tenant2,
                {self.key2: self.value2}))
...
class TestTenantQueue(testlib_api.SqlTestCase):
    ...
    def test_get_nwa_tenant_queues_no_result_found(self):
        self.ssn.query().all.side_effect = NoResultFound
        self.assertFalse(
            db_api.get_nwa_tenant_queues(self.ssn))

    def test_del_tenant_queue_no_result_found(self):
        self.ssn.query().filter().delete.side_effect = NoResultFound
        self.assertFalse(
            db_api.del_nwa_tenant_queue(self.ssn, self.tenant1))

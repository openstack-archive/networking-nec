...
class TestAddNwaTenantBinding(testlib_api.SqlTestCaseLight):
    ...
    def test_add_nwa_tenant_binding_no_result_found(self):
        ...
        self.assertFalse(
            db_api.add_nwa_tenant_binding(
                self.session, self.tenant1, self.nwa_tenant2,
                {self.key2: self.value2}))
...
class TestTenantQueue(testlib_api.SqlTestCase):
    ...
    def test_add_tenant_queue_no_result_found(self):
        ...
        self.session.query().filter().all.side_effect = NoResultFound
        self.assertFalse(
            db_api.add_nwa_tenant_queue(self.session, self.tenant1))
    ...
    def test_get_nwa_tenant_queues_no_result_found(self):
        self.session = MagicMock()
        self.session.query().all.side_effect = NoResultFound
        self.assertFalse(
            db_api.get_nwa_tenant_queues(self.session))

    def test_del_tenant_queue_no_result_found(self):
        self.session = MagicMock()
        self.session.query().filter().delete.side_effect = NoResultFound
        self.assertFalse(
            db_api.del_nwa_tenant_queue(self.session, self.tenant1))

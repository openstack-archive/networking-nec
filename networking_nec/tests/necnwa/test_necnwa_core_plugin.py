...
class TestNECNWACorePlugin:

    @patch('neutron.plugins.necnwa.necnwa_core_plugin.NECNWACorePlugin._setup_rpc')
    ...
    def test_extend_network_dict_provider_segment_none(self, f1):
        ...
        result = self.core_plugin._extend_network_dict_provider(context, network)
    ...
    @patch('neutron.plugins.necnwa.necnwa_core_plugin.NECNWACorePlugin._extend_network_dict_provider')
    @patch('neutron.plugins.ml2.plugin.Ml2Plugin.get_network')
    def test_get_network(self, f1, f2):
    ...
    def test_get_networks(self, f1):
        ...
        self.core_plugin.get_networks(context, filters, fields,
                                      sorts, limit, marker,
                                      page_reverse)
...
class TestNWARpcCallbacks:
    ...
    def setUp(self, f1, f2):
        ...
        self.rpc = NWARpcCallbacks(notifier, type_manager)
    ...
    @patch('neutron.plugins.necnwa.necnwa_core_plugin.NECNWACorePlugin.get_bound_port_context')
    @patch('neutron.plugins.necnwa.necnwa_core_plugin.NECNWACorePlugin._device_to_port_id')
    ...
    @patch('neutron.plugins.necnwa.necnwa_core_plugin.NECNWACorePlugin._device_to_port_id')
    @patch('neutron.manager.NeutronManager.get_plugin')
    def test_get_device_details_no_segment(self, f1, f2, f3, f4, f5):
        rpc_context = MagicMock()
        f1.return_value = self.core_plugin
        f2.return_value = None
        f3.current = True
    ...
    @patch('neutron.plugins.necnwa.necnwa_core_plugin.NECNWACorePlugin._device_to_port_id')
    @patch('neutron.manager.NeutronManager.get_plugin')
    def test_get_device_details_multi_segment(self, f1, f2, f3, f4, f5):
        rpc_context = MagicMock()
        f1.return_value = self.core_plugin
        f2.return_value = None
        f3.current = True
    ...
    @patch('neutron.plugins.necnwa.necnwa_core_plugin.NECNWACorePlugin._device_to_port_id')
    @patch('neutron.manager.NeutronManager.get_plugin')
    def test_get_device_details_segment_size_miss_match(self, f1, f2, f3, f4, f5):
        rpc_context = MagicMock()
        f1.return_value = self.core_plugin
        f2.return_value = None
        f3.current = True
    ...
    @patch('neutron.plugins.necnwa.necnwa_core_plugin.NECNWACorePlugin._device_to_port_id')
    @patch('neutron.manager.NeutronManager.get_plugin')
    def test_get_device_details_segment_zero(self, f1, f2, f3, f4, f5):
        rpc_context = MagicMock()
        f1.return_value = self.core_plugin
        f2.return_value = None
        f3.current = True
    ...
    @patch('neutron.plugins.necnwa.necnwa_core_plugin.NECNWACorePlugin._device_to_port_id')
    @patch('neutron.manager.NeutronManager.get_plugin')
    def test_get_device_details_segment_not_binding(self, f1, f2, f3, f4, f5, f6):
        rpc_context = MagicMock()
        f1.return_value = self.core_plugin
        f2.return_value = None
        f3.current = True
    ...
    @patch('neutron.plugins.necnwa.necnwa_core_plugin.NECNWACorePlugin.update_port_status')
    @patch('neutron.plugins.necnwa.db.api.ensure_port_binding')
    @patch('neutron.plugins.ml2.db.get_network_segments')
    @patch('neutron.db.api.get_session')
    @patch('neutron.plugins.necnwa.necnwa_core_plugin.NECNWACorePlugin.get_bound_port_context')
    @patch('neutron.plugins.necnwa.necnwa_core_plugin.NECNWACorePlugin._device_to_port_id')
    @patch('neutron.manager.NeutronManager.get_plugin')
    def test_get_device_details_bound_segment(self, f1, f2, f3, f4, f5, f6,f7):
    ...
    @patch('neutron.plugins.necnwa.necnwa_core_plugin.NECNWACorePlugin._device_to_port_id')
    @patch('neutron.manager.NeutronManager.get_plugin')
    def test_get_device_details_port_state_change(self, f1, f2, f3, f4, f5, f6, f7):

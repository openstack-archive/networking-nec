...
from networking_nec.plugins.necnwa.necnwa_plugin import (  # noqa
    NECNWAServicePlugin
)
...
class TestNECNWAServicePlugin(base.BaseTestCase):

    @patch('neutron.services.l3_router.l3_router_plugin.'
           'L3RouterPlugin.__init__')
    def setUp(self, l3p):
        super(TestNECNWAServicePlugin, self).setUp()
        setup_global()
    ...
    @patch('networking_nec.plugins.necnwa.necnwa_plugin.NECNWAServicePlugin.'
           '_delete_nat')
    @patch('neutron.services.l3_router.l3_router_plugin.'
    ...
    def test_update_floatingip_raise_before_super_call(self):
        ...
        fid = 'uuid-fid-111'
        context.session.query().filter_by().one.side_effect = \
    ...
    def check_called(self, cstfp, param):
        ...
        if param.get('call_count', None):
            self.assertEqual(cstfp.call_count, param['call_count'])
